...
public class ValiImg {
		...
		static int width = 220, height = 50;	//60,20
		static int FontSize=40;
		static int padding_x=20;//字符串水平内间距
		static int charNum=4;//字符数
		...
		public InputStream getIn() {
			try {
				...
				g.setColor(getRandColor(200, 250));				//描绘字体颜色
				g.fillRect(0, 0, width, height);
				g.setFont(new Font("Arial", Font.PLAIN, FontSize));		//描绘字体大小
				g.setColor(getRandColor(160, 200));
				for (int i = 0; i < 200; i++) {					//干扰背景
				...
				for (int j = 0; j < charNum; j++) {
					...
					g.drawString(ch[index]+"",padding_x+char_gap*j, char_y);			//描绘字体位置
					str.append(ch[index]);
				}
