...
$(document).ready(function(){
  ...
  $.ajax({
    ...
    success: function(data){
      ...
      var highFormat='{"chart": {"type": "column"}, "xAxis": {"categories": [""] }, "yAxis": {"min": 0, "title": {"text": "答题数"} }, '
    //  加载图片
      for(var i=0;i<$(".img").length;i++){
          var q_id=$($(".img")[i]).prev().prev().prev().text();
          q_id=q_id.substring(0,q_id.length-1);
          $.ajax({
    		  url: "../../ans_getAnsOnlyDataById",
    		  type: "POST",
    		  async:false, 
    		  data: {
    		  	'q.id':arr[1],
    		  ...
    		  success: function(data){
    		        $($(".img")[i]).highcharts(JSON.parse(highFormat+JSON.stringify(data).substr(1)));
    		  },
    		  error: function(){
    		    layer.msg('访问失败', hint);
    		  }
