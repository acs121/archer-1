...
/**
 * 根据查询的answerList生成Map<Integer,Integer> 选项号（A、B..）--选择数量
 * @param list
 * @return
 */
	public static Map<Integer, Integer> getAnsMap(List<String> list){
		Map<Integer, Integer> m=new HashMap<Integer,Integer>();
		for(String a:list){
			for(char c:a.toCharArray()){
				Integer option=m.get(c-'0');
				m.put(c-'0',(option==null)?1:(option+1));
			}
		}
		return m;
	}
