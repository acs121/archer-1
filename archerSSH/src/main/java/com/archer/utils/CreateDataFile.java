...
public class CreateDataFile {
  ...
  public static InputStream createTextPOI(List<String> sts){
      ...
      try {
			ansSheet.creatAuditSheet(workbook, "答卷信息", ans, RECORES_COLUMNS, RECORES_FIELDS);
   ...
   */
  public static InputStream createChioceDataPic(Map<Integer, Integer> map,int quesId,int type){
	 switch(type){
   ...
   * @return
   */
  private static InputStream createHistogram(Map<Integer, Integer> map,int quesId){
	  DefaultCategoryDataset dataset = new DefaultCategoryDataset();
	  for(int key : map.keySet()){
		  char c=(char) ('A'+key-1);
         dataset.addValue(map.get(key), "选项", String.valueOf(c));
	  }
		//工厂模式
 ...
 * @return 
   */
  private static InputStream createPie(Map<Integer, Integer> map,int quesId){
		//图形的数据集合
		DefaultPieDataset dataset = new DefaultPieDataset(); 
		for(int key : map.keySet()){
			  char c=(char) ('A'+key-1);
		         dataset.setValue(String.valueOf(c),map.get(key));
		}
		//工厂模式
